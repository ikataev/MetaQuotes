...
export class UIModel {
	...
	setStartYear(startYear: number): void {
		if (this._endYear && startYear > this._endYear) {
			throw new Error(`You can't set start year more then end, startYear: ${startYear}, endYear: ${this._endYear}`)
		}
	...
	setEndYear(endYear: number): void {
		if (this._startYear && endYear < this._startYear) {
			throw new Error(`You can't set end year less then start, startYear: ${this._startYear}, endYear: ${endYear}`)
		}
